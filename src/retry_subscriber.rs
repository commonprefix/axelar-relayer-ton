...
impl<TM: Retriable + AtomicUpsert, CL: RestClient> TransactionPoller for RetryTONSubscriber<TM, CL> {
    ...
    async fn poll_account(
        ...
        for trace in traces {
            ...
            if trace_model.is_incomplete {
                ...
                if retry <= 1 {
                    warn!("Trace {} is still incomplete and retry count is {}, skipping", trace.trace_id, retry);
                } else {
                    info!("Trace {} still incomplete, skipping", trace.trace_id);
                }
            } else if self.ton_trace_model.upsert_and_return_if_changed(trace_model).await?.is_some() && !trace.is_incomplete {
